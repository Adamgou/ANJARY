...
class MailMessage(models.Model):
    ...
    def cron_delete_discussion(self):
        ...
        channel_ids = self.env['discuss.channel'].search([]).filtered(
            lambda channel: channel.member_count > 2)

        for partner_id in partner_ids:
            records = MailMessage.search([('model', '=', 'discuss.channel'), ('res_id', 'not in', channel_ids.mapped('id')), ('author_id', '=', partner_id.id), (
        ...
        for channel_id in channel_ids.filtered(lambda channel: channel.is_message_deletion_active):
            records = MailMessage.search([('model', '=', 'discuss.channel'), ('res_id', '=', channel_id.id), (
