...
class ResPartner(models.Model):
        ...
        for partner_id in self:
            if (
                ...
                raise ValidationError(
                    _("This prefix is aleardy taken, please use another new one.")
                )
    ...
    def compute_sequence_started(self):
        ...
        for partner_id in self:
            if not partner_id.invoice_sequence_code:
                ...
                partner_id.sequence_started = (
                    IrSequence.sudo()
                    .search([("code", "=", partner_id.invoice_sequence_code)])[0]
                    .number_next_actual
                    > 1
                )
    ...
    def create(self, vals):
        ...
        filtered_partner_ids = partner_ids.filtered(
            lambda partner_id: partner_id.invoice_prefix is not False
        )
    ...
    def write(self, vals):
        ...
        if "invoice_prefix" in vals:
            self.filtered(
                lambda partner_id: partner_id.invoice_sequence_code is False
            )._set_invoice_sequence_code()
    ...
    def _set_invoice_sequence_code(self):
        for partner_id in self:
            partner_id.invoice_sequence_code = (
                f'{partner_id.name.lower().replace(" ", ".")}-{partner_id.id}'
            )
    ...
    def set_to_previous_sequence(self):
        sequence_id = (
            ...
            .sudo()
            .search([("code", "=", self.invoice_sequence_code)])
        )
